...
class DQNAgent:
    ...
    def train(self, num_frames: int, plotting_interval: int = 200):
        ...
        plt.savefig('results/2_double.png')
        self.env.close()
    ...
    def _compute_dqn_loss(self, samples: Dict[str, np.ndarray]) -> torch.Tensor:
        ...
        #DQN,自导自演，DQN根据Qvalue选择动作
        #Double DQN分离了选择和评估，DQN根据Qvalue选择动作，但再由target DQN评估该动作的Q value

        #n原DQN公式：next_q_value = self.dqn_target(next_state).max(dim=1, keepdim=True)[0].detach()
